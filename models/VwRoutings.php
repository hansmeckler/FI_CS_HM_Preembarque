...
/**
 * This is the model class for table "vw_preeroutings".
 *
 * The followings are the available columns in table 'vw_preeroutings':
 ...
 */
class VwRoutings extends CActiveRecord
{
	/**
...
//'cargoRouting' => array(self::HAS_ONE, 'CargosRouting', 'id_routing'),
'usuario0' => array(self::BELONGS_TO, 'UsuariosEmpresas', 'id_usuario_creacion'),
	...
	public function search()
	{
		...
		//if (isset($_GET['pais'])) 
		//	$criteria->compare('t.id_pais',$_GET['pais']);	
		

		$condition = Routings::actionWhere(2);
		$criteria->addCondition($condition);

		//$criteria->compare('case when cast("trackingsLast".fecha_alerta as date) = cast(now() as date) then 0 else 1 end',$this->fecha_alerta);
		...
		$criteria->compare('no_bl_id',$this->no_bl);*/
		$criteria->compare('bl_id_fecha',$this->bl_id_fecha,true,'ILIKE');
		Yii::app()->session['VwRoutings_records'] = $criteria;
	 ...
	 * @return VwRoutings the static model class
	 */
